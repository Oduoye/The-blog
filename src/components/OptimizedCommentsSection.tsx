...
const OptimizedCommentsSection = ({ postId, onLike, onShare, onComment }: OptimizedCommentsSectionProps) => {
  ...
  const handleAddComment = async () => {
    ...
    if (!engagement.globalCommentName && !authorName.trim()) {
      toast({
        title: "Error",
    ...
    try {
      await addComment(authorName.trim(), newComment.trim(), authorEmail.trim() || undefined);
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <Button 
            ...
            disabled={isSubmitting || !newComment.trim() || (!engagement.globalCommentName && !authorName.trim())}
